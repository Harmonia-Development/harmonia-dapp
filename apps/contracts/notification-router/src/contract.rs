use soroban_sdk::{contract, contractimpl, log, panic_with_error, symbol_short, Env, Symbol, Vec};
...
impl NotificationRouterContract {
    ...
    pub fn log_governance_event(env: Env, title: Symbol, message: Symbol, severity: Symbol) {
        ...
        env.events().publish(
            ...
            (category::GOVERNANCE, severity, title, message),
        );
    }
    ...
    pub fn log_treasury_event(env: Env, title: Symbol, message: Symbol, severity: Symbol) {
        ...
        env.events().publish(
            ...
            (category::TREASURY, severity, title, message),
        );
    }
    ...
    pub fn log_member_event(env: Env, title: Symbol, message: Symbol, severity: Symbol) {
        ...
        // Log the event
        env.events().publish(
            (symbol_short!("notify"), category::MEMBER, severity.clone()),
            (category::MEMBER, severity, title, message),
        );
    }
    ...
    pub fn log_system_event(env: Env, title: Symbol, message: Symbol, severity: Symbol) {
        ...
        // Log the event
        env.events().publish(
            (symbol_short!("notify"), category::SYSTEM, severity.clone()),
            (category::SYSTEM, severity, title, message),
        );
    }
    ...
    pub fn batch_emit(env: Env, category: Symbol, notifications: Vec<(Symbol, Symbol, Symbol)>) {
        ...
        for notification in notifications.iter() {
            ...
            env.events().publish(
                (symbol_short!("notify"), category.clone(), severity.clone()),
                (category.clone(), severity, title, message),
            );
        }
    }
