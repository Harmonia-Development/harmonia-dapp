...
use super::*;
use soroban_sdk::{
    contract, contractimpl, symbol_short, testutils::{Address as AddressTestUtils, Ledger},
    Address, Env, Symbol,
};
...
impl MockProposalContract {
    ...
    pub fn milestone_exists(env: Env, milestone_id: Symbol) -> bool {
        env.storage().persistent().get(&milestone_id).unwrap_or(false)
    }
...
fn test_schedule_release_insufficient_funds() {
    ...
    let result = client.try_schedule_release(&user, &asset, &1000, &(current_time + 100), &milestone_id);
...
fn test_milestone_based_release() {
    ...
    assert_eq!(treasury_client.get_balance(&asset), deposit_amount - milestone_amount);
...
fn test_multi_milestone_proposal() {
    ...
    assert_eq!(treasury_client.get_reserved(&asset), payment_1 + payment_2 + payment_3);
    ...
    assert_eq!(treasury_client.get_balance(&asset), deposit_amount - payment_1 - payment_2);
    ...
    assert_eq!(treasury_client.get_balance(&asset), deposit_amount - payment_1 - payment_2 - payment_3);
    assert_eq!(treasury_client.get_reserved(&asset), 0);
}
