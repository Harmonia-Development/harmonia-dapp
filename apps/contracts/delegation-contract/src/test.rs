...
use delegation_contract::{DelegationContract, DelegationContractClient, DelegationError};

fn create_test_env() -> (Env, DelegationContractClient, Address, Address, Address, Address) {
    let env = Env::default();
    let admin = Address::generate(&env);
    let contract_id = env.register_contract(DelegationContract {},(admin.clone()));
...
fn test_multiple_delegators() {
    let (env, client, member_1, member_2, member_3, member_4) = create_test_env();
    ...
    // member_2 should have: 5 (base) + 10 (member_1) + 3 (member_3) + 2 (member_4) = 20
    assert_eq!(client.get_voting_power(&member_2), 20);
    ...
    let delegator_addrs: Vec<Address> = delegators.iter().collect();
