...
const cardVariants = cva("rounded-lg p-4 md:p-5 relative", {
  variants: {
    variant: {
      default:
        "bg-gradient-to-br from-gray-darker to-gray-dark border border-gray-dark",
      success:
        "bg-gradient-to-br from-green-900/40 to-green-950/40 border border-success/20",
      error:
        "bg-gradient-to-br from-red-900/40 to-red-950/40 border border-danger/20",
...
const renderIcon = (
  ...
  return (
    <div className={iconContainerVariants({ variant })}>
      <span
        className={clsx(currentClassName, iconVariants({ variant }), "w-4 h-4")}
      >
        {icon}
      </span>
    </div>
