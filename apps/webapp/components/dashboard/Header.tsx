  ...
  return (
    <header className="bg-black text-white px-6 py-5 shadow-xl  flex items-center justify-between border-b border-gray-500">
        ...
        <nav className="hidden md:flex items-center space-x-2">
          {navItems.map((item) => (
            ...
            >
              ...
              <span>{item.name}</span>
              {item.name === 'Analytics' && (
                <span className="absolute w-2 h-2 bg-white rounded-full -mt-5 animate-pulse"></span>
              )}
            </Link>
          ))}
        </nav>
      ...
      <div className="flex items-center space-x-2">
        ...
        {/* Notifications */}
        <button className="p-2 rounded-md text-gray-300 hover:bg-gray-700 hover:text-white relative">
          <Bell size={20} />
