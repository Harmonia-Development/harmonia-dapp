  ...
  return [
    {
      ...
      message:
        "Community Garden initiative voting ends in 24 hours. Cast your vote now!",
      time: "1 hour ago",
      priority: "High",
      category: "Governance",
      from: "@governance.dao",
...
const PriorityBadge = ({ priority }: { priority: string }) => {
  ...
  return (
    <span className={`${getColor()} text-xs px-2 py-1 rounded-md font-medium`}>
      {priority}
    </span>
  );
};
  ...
  return (
    <div className="p-4 bg-gradient-to-r from-purple-900/30 to-purple-950/40 rounded-lg flex gap-3">
      <div className="flex-shrink-0 mt-1">
        <Icon
          className={`h-5 w-5 ${getPriorityColor(notification.priority)}`}
        />
      </div>
      <div className="flex-grow min-w-0">
        ...
        <div className="flex justify-between text-xs text-gray-400">
          ...
          <span className="flex items-center gap-1">
            ...
            {!notification.from.includes("@") &&
              notification.from === "stellar.dev" && (
