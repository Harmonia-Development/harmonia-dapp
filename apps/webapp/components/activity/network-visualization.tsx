	...
	return (
		...
		>
			...
			{hovered && (
				...
				>
					<p>{id}</p>
					{connections.length > 0 ? <p>{`connections: [${connections.join(', ')}]`}</p> : null}
				</Html>
			)}
		</mesh>
	)
	...
	useFrame(({ clock }) => {
		if (ref.current) {
			if (ref.current.material instanceof THREE.Material) {
				ref.current.material.opacity = 0.3 + Math.sin(clock.getElapsedTime() * 1.5) * 0.1
			}
		}
	})
...
function GridSphere() {
	...
	return (
		<lineSegments ref={gridRef}>
			...
			<lineBasicMaterial attach="material" color="#3a2a7d" transparent opacity={0.15} />
		</lineSegments>
	)
}
